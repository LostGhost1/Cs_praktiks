...
namespace Serialize_People
{
  ...
  class Program
  {
    ...
    {
      // Создаемфайлдлясохраненияданных
      try
      {
        FileStream fs = new FileStream("Person.XML", FileMode.Create);
      
        // СоздаемобъектXmlSerializerдлявыполнениясериализации
        XmlSerializer xs = new XmlSerializer(typeof(Person));

        // Используем объект XmlSerializerдля сериализации данных в файл
        xs.Serialize(fs, sp);
    ...
    private static Person Deserialize()
    {
      ...
      // СоздаемобъектXmlSerializerдлявыполнениядесериализации
      XmlSerializer xs = new XmlSerializer(typeof(Person));
      // Используем объект XmlSerializerдля десериализации данных в файл
